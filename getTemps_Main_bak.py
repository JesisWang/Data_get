...
# sta_data_date="2019-04-16 15:30:00"
# end_data_date="2019-04-16 18:40:00"
#定时输入函数！！！
def readInput( caption, default, timeout = 5):
    ...
    while True:
        ...
        if len(input) == 0 and (time.time() - start_time) > timeout:
            print("\n timing out, using default value.")
            break
...
def getStackInfo(que,sta_i,sta_codes,sta_names):
    ...
    mkdir('./temp_datas/'+str(sta_i)+'_'+sta_names[sta_i])
    #默认 0 -1
    sbox,ebox=0,-1
    sba,eba=0,-1
    scl,ecl=0,-1
    sp,ep=0,-1
    #默认以包，簇，堆
    mode=['getDTempsByPack','getDTempsByCL','getDTempsByBA']
    
    rs=obj.getCellDataByPack(sbox=sbox,ebox=ebox,sba=sba,eba=eba,scl=scl,ecl=ecl,sp=sp,ep=ep,mode=mode)
    ...
    if ep==-1:
        ep=obj.pack_num
#     if eba==-1:
#         eba=obj.ba_num1#以第一箱的为基准
#     if ecl==-1:
#         ecl=obj.cl_num1#以第一堆的为基准
...
#     rs=[[[25.0, 23.0, 2.0, 24.333333333333332], [25.0, 23.0, 1.0, 24.166666666666668], [26.0, 23.0, 1.0, 25.166666666666668], [25.0, 23.0, 1.0, 24.166666666666668], [25.0, 23.0, 1.0, 24.833333333333332], [25.0, 24.0, 1.0, 25.0], [25.0, 23.0, 1.0, 24.666666666666668], [25.0, 23.0, 2.0, 24.666666666666668], [25.0, 23.0, 1.0, 24.666666666666668], [25.0, 23.0, 1.0, 24.666666666666668], [25.0, 23.0, 1.0, 24.666666666666668], [25.0, 23.0, 1.0, 24.833333333333332], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]], [[26.0, 23.0, 2.0, 24.555555555555557], [26.0, 23.0, 2.0, 24.62962962962963], [25.0, 23.0, 2.0, 24.555555555555557], [25.0, 23.0, 2.0, 24.77777777777778], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]], [[26.0, 23.0, 2.0, 24.970807875084862], [[], [], [], []]]]
#     ecl=3
...
#     obj.cl_num_list=[4,5]
    ...
    if len(rs)==len(mode):
        write=pd.ExcelWriter("./temp_datas/"+str(sta_i)+'_'+sta_names[sta_i]+'/'+sta_data_date.split(' ')[0]+".xlsx")
        for i in range(len(rs)):
            ...
            for box in range(sbox+1,ebox+1):
                ...
                for ba in range(sba+1,eba+1):
                    xba=str(ba)+'堆'
                    A=sum(obj.ba_num_list[0:box-1-sbox])#(box-sbox-1)*(eba-sba)
                    if mode[i]=='getDTempsByBA':
                        ...
                        if ecl==-1:
                            ecl=obj.cl_num_list[ba-1-sba+A]
                            print(ecl)
                            FLag=True
                        for cl in range(scl+1,ecl+1):
                            xcl=xba+'-'+str(cl)
#                             B=(A+(ba-sba-1))*(ecl-scl)
                            B=sum(obj.cl_num_list[0:ba-1-sba+A])
...
#             print(df)
            ...
            try:
                ...
                ax2.set_ylabel('箱')
                plt.savefig("./temp_datas/"+str(sta_i)+'_'+sta_names[sta_i]+'/'+sta_data_date.split(' ')[0]+txt+'最高温度与最大温差.png')
            except:
                print('图像绘制失败')
        write.save()
    print(que.get())
...
if __name__ == '__main__':
    ...
    ins=readInput('请在60s内输入编号(如新丰电站编号为0,多个使用英文逗号隔开)：', '5,30,23,13,25,32,33,0,8,2,9,3,10,4,7,6,1,31', timeout=60)
    ns=ins.split(',')
    runFlag=True
    for item in ns:
        try:
